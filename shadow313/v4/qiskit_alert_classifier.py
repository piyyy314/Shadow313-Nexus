...
class QiskitAlertClassifier:
    ...
    def __init__(
        ...
        if HAS_QISKIT:
            self._setup_quantum()
        elif use_classical_fallback and HAS_SKLEARN:
            self._setup_classical()
        else:
            raise RuntimeError(
                "Neither Qiskit nor scikit-learn available. "
                "Run: pip install qiskit qiskit-machine-learning scikit-learn"
            )
    ...
    def _setup_quantum(self):
        ...
        self._backend = "Qiskit StatevectorSampler (exact simulation)"

    def _setup_classical(self):
        """Initialize classical SVM fallback."""
        self._qsvc = SVC(kernel="rbf", probability=True, random_state=self.random_state)
        self._backend = "Classical SVM (RBF kernel) — Qiskit not available"
    ...
    def fit(
        ...
        if verbose:
            print(f"[QSVC] Training on {len(X_proc)} samples")
            print(f"[QSVC] Backend: {self._backend}")
            print(f"[QSVC] Classes: {[SEVERITY_LABELS.get(c, c) for c in self._classes]}")
            if HAS_QISKIT:
                print(f"[QSVC] Feature map: ZZFeatureMap(dim={self.n_features}, reps={self.reps})")
                print(f"[QSVC] Computing quantum kernel matrix ({len(X_proc)}×{len(X_proc)})...")
        ...
        if verbose:
            ...
            print(f"[QSVC] Training accuracy: {train_acc:.3f}")
            print(f"[QSVC] Training complete.")
    ...
    def evaluate(self, X: np.ndarray, y_true: np.ndarray) -> dict:
        ...
        return {
            ...
            "feature_map": f"ZZFeatureMap(dim={self.n_features}, reps={self.reps})" if HAS_QISKIT else "RBF kernel",
            "n_training_samples": "≤200 (quantum kernel scale constraint)" if HAS_QISKIT else "unlimited",
            "honest_note": (
    ...
    def circuit_diagram(self) -> str:
        ...
        return "Qiskit not available — using classical SVM fallback"

    def __repr__(self) -> str:
        status = "fitted" if self.is_fitted else "unfitted"
        return (f"QiskitAlertClassifier("
                f"n_features={self.n_features}, reps={self.reps}, "
                f"backend={'quantum' if HAS_QISKIT else 'classical'}, "
                f"status={status})")